...
var precedencese = map[token.TokenType]int{
	token.EQ:       EQUALS,
	token.NOT_EQ:   EQUALS,
...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerInfix(token.PERSENT, p.parseInfixExpression)
...
func (p *Parser) peekPrecedence() int {
	if p, ok := precedencese[p.peekToken.Type]; ok {
		return p
	}
...
func (p *Parser) curPrecedence() int {
	if p, ok := precedencese[p.curToken.Type]; ok {
		return p
	}
...
func (p *Parser) parseInfixExpression(left ast.Expression) ast.Expression {
	...
	precedencese := p.curPrecedence()
	p.nextToken()
	expression.Right = p.parseExpression(precedencese)
